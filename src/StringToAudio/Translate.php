...
use Karmadice\StringToAudio\Token\TokenGenerator;
use Karmadice\StringToAudio\Token\TokenProviderInterface;
...
class Translate
{
    ...
    public function __construct(string $target = 'en', string $source = null, array $options = null, TokenProviderInterface $tokenProvider = null)
    {
        ...
        $this->setTokenProvider($tokenProvider ?? new TokenGenerator())
    ...
    {
        return (new self)
            ->setTokenProvider($tokenProvider ?? new TokenGenerator())
            ->setOptions($options) // Options are already set in client constructor tho.
            ->setSource($source)
            ->setTarget($target)
            ->translate($string);
